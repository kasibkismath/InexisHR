...
reports.controller('reportsMainController', ['$scope', '$http', '$q', 'toaster', '$filter',
	...
	$scope.init = function(){
		...
		$scope.appraisalsReportOption = DTOptionsBuilder.newOptions()
        .withButtons([
		               {
		            	   extend: 'print',
		...
		$scope.vacanciesReportOption = DTOptionsBuilder.newOptions()
        .withButtons([
		               {
		            	   extend: 'print',
		...
		$scope.trainingsReportOption = DTOptionsBuilder.newOptions()
	    .withButtons([
		               {
		            	   extend: 'print',
	...
	$scope.generateVacanciesReport = function() {
		$http.get($scope.baseURL + '/Reports/GenerateVacanciesReport')
		.success(function(result) {
			$scope.generateVacanciesReportResult = result;
		})
		.error(function(data, status) {
			console.log(data);
		});
	...
	$scope.generateTrainingsReport = function(fromDate, toDate, typeOfBudget) {
		
		console.log(typeOfBudget);
		
		var trainings = {
			fromDate: fromDate,
			toDate: toDate,
			type_of_budget: typeOfBudget
		};
		$http.post($scope.baseURL + '/Reports/GenerateTrainingsReport', trainings)
		.success(function(result) {
			$scope.generateTrainingsReportResult = result;
			$scope.getTotal = function(){
		        var total = 0;
		        for(var i = 0; i < $scope.generateTrainingsReportResult.length; i++){
